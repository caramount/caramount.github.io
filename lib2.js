...
function gl_init(gl, vertexShader, fragmentShader) {
   ...
   gl.ambient  = gl.getUniformLocation(program, "ambient" );
   gl.diffuse  = gl.getUniformLocation(program, "diffuse" );
   gl.specular  = gl.getUniformLocation(program, "specular" );
   gl.power  = gl.getUniformLocation(program, "power" );
}
...
function gl_update(gl) {
   ...
   gl.uniform4fv(gl.uSphere  , sphereValues);
   gl.uniform3fv(gl.ambient  , ambient);
   gl.uniform3fv(gl.diffuse  , diffuse);
   gl.uniform3fv(gl.specular , specular);
   gl.uniform1f(gl.power     , power);
