...
Matrix.prototype = {
   ...
   rotateX : function(ang) {
      this._makeRotationX(this._tmp1, ang);
      this._multiply(this._tmp1);
   },

   rotateY : function(ang) {
      this._makeRotationY(this._tmp1, ang);
      this._multiply(this._tmp1);
   },

   rotateZ : function(ang) {
      this._makeRotationZ(this._tmp1, ang);
      this._multiply(this._tmp1);
   },
   ...
   _makeRotationX : function(dst, ang) {
      this._makeIdentity(dst);
      dst[1][1] = Math.cos(ang);
      dst[1][2] = Math.sin(ang) * -1;
      dst[2][1] = Math.sin(ang);
      dst[2][2] = Math.cos(ang);
   },

   _makeRotationY : function(dst, ang) {
      this._makeIdentity(dst);
      dst[0][0] = Math.cos(ang);
      dst[0][2] = Math.sin(ang);
      dst[2][0] = Math.sin(ang) * -1;
      dst[2][2] = Math.cos(ang);
   },

   _makeRotationZ : function(dst, ang) {
      this._makeIdentity(dst);
      dst[0][0] = Math.cos(ang);
      dst[0][1] = Math.sin(ang) * -1;
      dst[1][0] = Math.sin(ang);
      dst[1][1] = Math.cos(ang);
   },
